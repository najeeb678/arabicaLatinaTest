...
const ShawlsMainPage = () => {
  ...
  const {
    productData: { shawls },
    colorsData: { shawls: shawlsColors },
    loading: { shawls: isShawlsLoading },
    errors: { shawls: scarvesError },
  ...
  useEffect(() => {
    if (isFlagProcessed) {
      ...
      dispatch(fetchShawlsProductsColors(colorParams)).unwrap(),
  ...
  const filtersData = useMemo(
    () => [
      {
        label: "Color",
        options: shawlsColors?.regular || [],
      },
      {
        label: "Duotone",
        options: shawlsColors?.duotone || [],
      },
      {
        label: "Style",
  ...
  return (
    <>
      <Head>
        <title> Elegant Shawls For Men Women - Arabica Latina</title>
        <meta
          name="description"
